...
packages <- c("ggplot2", "dplyr", "tidyverse", "tidyr", "geosphere",
              ...
              "ggrepel","DT","forecast","maps","usmap","clue",
...
colnames(forecast) = c("city_index","forecast_date","forecast_value","forecast_goal",
                       "forecast_made_on")
## We also provide rds format of forecast dataset, if you want to use that instead, please comment the above two line and run the code below
## But we have not tried using this instead, so some error may appear in the latter code related to this data and may need some code adjusting
...
mindiff_day1_nona <- read_csv("mindiff_day1_nona.csv")
mindiff_state1_smooth <- read_csv("mindiff_state1_smooth.csv")

smooth_curve_df = bind_cols(mindiff_day1_nona, mindiff_state1_smooth) %>%
  rename(date = x)

curve_plot_df = smooth_curve_df %>% 
  pivot_longer(
...
overall_mean_curve = smooth_curve_df %>% 
  mutate(avg_temp = apply(smooth_curve_df[, 2:51], 1, mean)) %>% 
  dplyr::select(date, avg_temp)

fig_3 = curve_plot_df %>% 
  ggplot() +
  geom_line(aes(x = date, y = temp, group = state), col = "gray80") + 
  geom_line(data = overall_mean_curve, 
            aes(x = date, y = avg_temp), col = "red") + 
  theme_light() + 
  scale_x_date(date_breaks = "2 month", date_labels = "%Y-%m") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  ylab("Absolute prediction error (Â°F)") + xlab("Date")
fig_3
```
...
### Figure 6 ###
real_analysis_result <- read_csv("real_analysis_result.csv")
...
curve_plot_df = smooth_curve_df %>% 
  ...
  left_join(real_analysis_result %>% 
              dplyr::select(state, cluster = fem.4) %>% 
...
cluster_mean_curve = curve_plot_df %>% 
...
fig_6 = curve_plot_df %>% 
  ggplot() +
  geom_line(aes(x = date, y = temp, group = state), col = "gray80") + 
  geom_line(data = cluster_mean_curve, 
            aes(x = date, y = avg_temp), col = "red", lty = 2) + 
  theme_light() +
  scale_x_date(date_breaks = "2 month", date_labels = "%Y-%m") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(~cluster) + 
