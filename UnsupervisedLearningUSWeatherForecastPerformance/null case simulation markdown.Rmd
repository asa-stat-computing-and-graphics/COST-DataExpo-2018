...
This is the R file including all the content related to the simulated data based on Null case clustering (for the response of the reviewer 2). *Please notice that this is a simulation, so the execution time would be around 2.5 hours.* The main content of this file includes:
...
packages <- c("ggplot2", "dplyr", "tidyverse", "tidyr", "ggridges",
              ...
              "ggrepel","DT","ggthemes","fpc",
              "gridExtra","factoextra", "NbClust","funFEM","mapproj")
ipak(packages)
```

### 1. Functions for data generation and validation
...
### 2. Simulation Process and Cluster Number Detection Validation
```{r, message=FALSE, warning=FALSE}
...
for (i in  1:98){
  ...
  cluster.result$clu.num = cluster.select.num(simu.all=df,n.group=40,knots.num=10,nharm = 4,seed=seed,plotting = F)
...
for (i in  1:2){
  ...
  cluster.result$n.group = rep(40,each = 4)
  cluster.result$round = rep("by = 0.01, sd = 0.5, common noise",each = 4)
  cluster.result$clu.num = cluster.select.num(simu.all=df,n.group=40,knots.num=10,nharm = 3,seed=seed,plotting = F)
