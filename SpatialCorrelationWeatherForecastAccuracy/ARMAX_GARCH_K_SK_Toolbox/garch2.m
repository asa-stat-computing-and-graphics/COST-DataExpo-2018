function [parameters, stderrors, LLF, ht, resids, summary] = garch2(data, model, distr, ar, ma, x, p, q, y, startingvalues, options)
%{
-----------------------------------------------------------------------
 PURPOSE:  ***Slightly modified for Jang's Functional Time Series model***
...
%         upperbounds  = [ 1; ones(ar+ma,1); ux; ones(p+q+1,1); uy];
        lowerbounds  = [0; -ones(ar+ma,1); lx; 1e-8*ones(p+q+1,1); ly];
        upperbounds  = [0; ones(ar+ma,1); ux; inf; ones(p+q,1); uy];
    case 2 % GJR
